...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("heartbeat", "设置定时提示词，空闲时自动进入本会话", "会话",
               aliases=("hb",), args_hint="[every <interval> <prompt> | status | pause | resume | clear]",
               subcommands=("status", "pause", "resume", "clear"),
               busy_policy="dispatch"),
    CommandDef("refine", "立即复盘本对话并保存经验到记忆/技能", "会话",
               args_hint="[focus instructions]"),
    CommandDef("review", "派独立子助手审查刚讨论的工作（PR、代码、文档）", "会话",
               args_hint="[review instructions]"),
    CommandDef("loop", "在本会话中按固定间隔重复运行提示词", "会话",
               aliases=("proactive",),
    ...
    CommandDef("plan", "把实施计划写成 markdown 存到 .hermes/plans/（不执行任何操作）", "会话",
               args_hint="[task]"),
    CommandDef("moa", "用默认多模型协作预设运行一次提示词，然后恢复你的模型", "会话",
    ...
    CommandDef("focus", "切换专注视图——只显示你的提示词和最终回复",
               "配置", cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("footer", "在最终回复上切换网关运行时元数据页脚",
               "配置", args_hint="[on|off|status]",
               subcommands=("on", "off", "status"), busy_policy="dispatch",
               desktop="terminal"),
    CommandDef("yolo", "切换 YOLO 模式（跳过所有高危命令审批）",
               "配置", busy_policy="dispatch"),
    CommandDef("approvals", "显示或设置持久的高危命令审批模式",
               "配置", args_hint="[manual|smart|off]",
               subcommands=("manual", "smart", "off")),
    CommandDef("reasoning", "管理推理力度和显示", "配置",
               ...
               desktop="advanced"),
    CommandDef("fast", "切换快速模式——OpenAI 优先处理 / Anthropic 快速模式（normal/fast/auto/cold）", "配置",
    ...
    CommandDef("wake", "切换 'Hey Hermes' 唤醒词监听", "配置",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "控制 Hermes 工作时消息的行为", "配置",
    ...
    CommandDef("skills", "搜索、安装、检查或管理技能",
               ...
               desktop="settings"),
    CommandDef("memory", "查看待处理的记忆写入 / 切换审批开关",
               "工具与技能",
               args_hint="[pending|approve|reject|approval] [id|on|off]",
               subcommands=("pending", "approve", "reject", "approval")),
    CommandDef("bundles", "列出技能包（/<name> 别名对应多个技能）",
               "工具与技能", execute="bundles"),
    CommandDef("pet", "切换或领养宠物伙伴（/pet、/pet list、/pet <slug>）", "工具与技能",
               cli_only=True, args_hint="[toggle|list|scale <n>|<slug>]", subcommands=("toggle", "list", "scale", "off")),
    CommandDef("hatch", "根据描述生成新的宠物伙伴",
               "工具与技能", cli_only=True, aliases=("generate-pet",), args_hint="[description]"),
    CommandDef("learn", "从你描述的任何内容学习可复用技能（目录、网址、本对话、笔记）",
               "工具与技能", args_hint="<what to learn from>"),
    CommandDef("init", "从仓库扫描生成或更新 AGENTS.md 项目说明",
               "工具与技能", args_hint="[notes]"),
    CommandDef("cron", "管理定时任务", "工具与技能",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove"),
               desktop="terminal"),
    CommandDef("suggestions", "查看建议的自动化（接受/拒绝）",
               "工具与技能", aliases=("suggest",), args_hint="[accept|dismiss N | catalog]",
               subcommands=("accept", "dismiss", "catalog", "clear")),
    CommandDef("blueprint", "从蓝图模板设置自动化",
               "工具与技能", aliases=("bp",), args_hint="[name] [slot=value ...]"),
    CommandDef("curator", "后台技能维护（status、run、pin、archive、list-archived）",
               "工具与技能", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived"),
               desktop="advanced"),
    CommandDef("kanban", "多档案协作看板（任务、链接、评论）",
               "工具与技能", args_hint="[subcommand]",
    ...
    CommandDef("reload-skills", "重新扫描 ~/.hermes/skills/ 查找新安装或移除的技能",
               "工具与技能", aliases=("reload_skills",), desktop="advanced"),
    CommandDef("browser", "通过 CDP 将浏览器工具连接到你的 Chromium 系浏览器，或切换到 Browser Use 模式", "工具与技能",
    ...
    CommandDef("plugins", "列出已安装的插件及其状态",
               "工具与技能", cli_only=True, desktop="terminal"),

    # Info
    CommandDef("commands", "浏览所有命令和技能（分页）", "信息",
               gateway_only=True, args_hint="[page]", busy_policy="dispatch",
               execute="gateway_commands"),
    CommandDef("help", "显示可用命令（/help skills 列出技能命令，/help <text> 过滤）", "信息", busy_policy="dispatch",
               execute="gateway_help", args_hint="[skills|<filter>]"),
    CommandDef("palette", "打开模糊命令面板（也可用 Ctrl+P）", "信息",
               cli_only=True, busy_policy="dispatch"),
    CommandDef("restart", "排空活动任务后优雅重启网关", "会话",
               gateway_only=True, busy_policy="dispatch", desktop="terminal"),
    CommandDef("usage", "显示 token 用量和速率限制；`reset` 兑换已存的 Codex 限额重置", "信息",
               args_hint="[reset [--force]]"),
    CommandDef("subscription", "查看你的 Nous 套餐并在浏览器中更改", "信息",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "在门户查看你的 Nous 余额并管理账单", "信息"),
    ...
    CommandDef("platform", "暂停、恢复或列出故障的网关平台", "信息",
               gateway_only=True, args_hint="<pause|resume|list> [name]"),
    CommandDef("copy", "将最后一条助手回复复制到剪贴板", "信息",
               cli_only=True, args_hint="[number]", desktop="terminal"),
    CommandDef("paste", "从剪贴板附加剪贴板图片", "信息",
               cli_only=True, desktop="terminal"),
    CommandDef("image", "为下一条提示词附加本地图片文件", "信息",
    ...
    # Exit
    CommandDef("quit", "退出 CLI（使用 --delete 同时删除会话历史）", "退出",
...
# Help renderer sub-grouping: the "会话" category accumulated ~46 commands
...
# here falls under the base "会话" header. Names are bare (no leading /).
